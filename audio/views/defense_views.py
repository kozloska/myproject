...
class DefenseViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['GET'])
    def protocol_data(self, request, pk=None):
        """Получить данные для протокола защиты"""
        defense = self.get_object()
        ...
        for protocol in protocols:
            ...
            data.append({
                ...
                'project': {
                    ...
                    'Supervisor': project.Supervisor if project else None
                },
                'question': {
                    'ID': protocol.ID_Question.ID if protocol.ID_Question else None,
                    'Text': protocol.ID_Question.Text if protocol.ID_Question else None
                },
